...
public class EnemyBehavior : MonoBehaviour {
	...
	public void OnHit() {
		...
		if (HP < 1) {
			if (Random.Range (0, 100) < 30) {
				GameControl.intance.popupMgr.SetType (PopupMgr.POPUP_TYPE.ENEMY_HURT);
			} else {
				GameControl.intance.popupMgr.SetType (PopupMgr.POPUP_TYPE.PLAYER_SMILE);
			}
        ...
        IGMMng.instance.BossHead.text = HP.ToString();
    }
	...
	public Ground.GroundCollider GetFoot() {
		...
		return new Ground.GroundCollider (new Vector2(bound.min.x + (bound.max.x - bound.min.x)/2, bound.min.y), 85);
	}
	...
	public void SetMovingState(MOVING_STATE state) {
		...
		case MOVING_STATE.STILL:
			if (m_movingState != state) {
                Debug.Log("EnemyBehavior: MOVING_STATE.STILL");
		...
		case MOVING_STATE.MOVING:
			if (m_movingState != state) {
				Debug.Log("EnemyBehavior: MOVING_STATE.MOVING");
                    m_skeletonAnimation.loop = true;
		...
		case MOVING_STATE.DYING:
			if (m_movingState != state) {
				Debug.Log("EnemyBehavior: MOVING_STATE.DYING");
		...
		case MOVING_STATE.DIED:
			if (m_movingState != state) {
				Debug.Log("EnemyBehavior: MOVING_STATE.DIED");
			}
			break;
		}
		m_movingState = state;
	...
	public void Init() {
		...
		//m_skeletonAnimation.state.End += OnAnimationCompleted;

		m_bombController = GetComponent<BombController> ();
	}

	public void ResetGameplay() {
		GameControl.intance.popupMgr.SetType(PopupMgr.POPUP_TYPE.ENEMY_ANGRY);
		SetMovingState(MOVING_STATE.STILL);
		SetBombState(BOMB_STATE.READY);
