...
public class PixelExplode : MonoBehaviour {
    public PixelDestruct pD;
    CustomPhysics physics;
    //	CustomRayCast rayCast;

    /* Explode */
    // Creates an "explosion" by finding all pixels near a point and launching them away
    public void explode(int xPos, int yPos, float radius, float alpha)
    {
                    ...
                    if (y >= pD.World.Bounds.Min.y && y <= pD.World.Bounds.Max.y) // boundary check
                    {
                        ...
                        if (solid) // we know this pixel is solid, now we need to find out if it's close enough
                        {
                            ...
                            if (distSq < radiusSq)
                            {
                                ...
                                PixelParticle.CreateAndAddToPhysics(new Color(rand, rand, rand, alpha), new Vector2(x, y), new Vector2(velX, velY));
                                Profiler.EndSample();
                                ++particlesCreated;
